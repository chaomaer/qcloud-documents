...
Hive 的内部表和外部表：
- **内部表：**实际上是将 hdfs 的文件映射成 table，然后 Hive 的数据仓库会生成对应的目录，EMR 中默认的仓库路径为 `user/hive/warehouse/$tablename`，**这个路径在 hdfs 上面**，其中 `$tablename` 是您创建的表名。这时只要将符合 table 定义的文件加载到此目录中，即可通过 Hql（Hive-SQL）对整个目录的文件进行查询。
